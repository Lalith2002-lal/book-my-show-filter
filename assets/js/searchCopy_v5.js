...
const mainFunction = async function () {
  ...
  // Function for the filter by genre and the rating
  function filterByGenreAndRating(selectedGenre, selectedRating) {
    const filteredMovie = movies.filter((movie) => {
      const matchesGenre =
        selectedGenre === "" ||
        selectedGenre === "all" ||
  ...
  genreSelect.addEventListener("change", function () {
    const selectedGenre = this.value.toLowerCase();
    const selectedRating = ratingSelect.value;
    const filteredMovie = filterByGenreAndRating(selectedGenre, selectedRating);
